...
class GaussianDiffusion:
    ...
    def p_sample(
        ...
        if conf.inpa_inj_sched_prev:
            ...
            if pred_xstart is not None:
                ...
                gt_keep_mask2 = model_kwargs.get('gt_keep_mask2')
                gt_keep_mask3 = model_kwargs.get('gt_keep_mask3')
                ...
                gt_inpaint_mask = model_kwargs.get('gt_inpaint_mask')
                gt =   model_kwargs['gt']
                ...
                if conf.inpa_inj_sched_prev_cumnoise:
                    ...
                    gt_part = gt_weight * gt # 对于已知部分来说，gt就是x_0
    ...
    def p_sample_loop_progressive(
        ...
        if noise is not None:
            ...
            import random
            image_after_step = random.randint(0,20) * th.randn(*shape, device=device)
            image2_after_step = random.randint(0,20) * th.randn(*shape, device=device)
            image3_after_step = random.randint(0,20) * th.randn(*shape, device=device)

        gt_keep_mask = model_kwargs['gt_keep_mask']
        gt_keep_mask2 = model_kwargs['gt_keep_mask2']
        gt_keep_mask3 = model_kwargs['gt_keep_mask3']

        image_after_step = (gt_keep_mask2 * gt_keep_mask3 * image_after_step 
        + 
        (1 - gt_keep_mask2)* image2_after_step 
        + 
        (1 - gt_keep_mask3) * image3_after_step
        )
